...
def plot_and_save_waveforms(t, tx, rx, tx_start_time, rx_start_time, duration,
                            show=False, save=True, out_dir=None, filename_prefix="wavepic"):
    """
    绘制发射/接收波形，可显示并保存为图片。
    图片命名为 wavepic_实际时间.png，实际时间取计算机时钟（格式 YYYYmmdd_HHMMSS）。
    """
    ...
    if save:
        ...
        saved_path = os.path.join(out_dir, f"{filename_prefix}_{ts}.png")
        fig.savefig(saved_path, dpi=300)
        print(f"波形图片已保存: {saved_path}")
...
def demo_and_export(frequency=40000.0, noise_std=0.02, fs=500000.0,
                    ...
                    out_csv="waveforms.csv", plot=False, save_pic=True, pic_dir=None):
    """
    ...
    # 绘制并保存波形图（默认保存，同步可选显示）
    if plot or save_pic:
        out_dir = pic_dir or os.path.dirname(os.path.abspath(out_csv))
        plot_and_save_waveforms(t, tx, rx, tx_start_time, rx_start_time, duration,
                                show=plot, save=save_pic, out_dir=out_dir)
...
def parse_args():
    ...
    p.add_argument("--savepic", dest="savepic", action="store_true", default=True,
                   help="保存发射/接收波形图片 (wavepic_实际时间.png)")
    p.add_argument("--no-savepic", dest="savepic", action="store_false",
                   help="不保存波形图片")
    p.add_argument("--picdir", type=str, default=None,
                   help="波形图片保存目录（默认与 CSV 相同目录）")
    return p.parse_args()
